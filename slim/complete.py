...
tf.logging_set_verbosity(tf.logging.DEBUG)
...
def experiment_fn(run_config, params):
	...
	run_config = run_config.replace(
		saved_checkpoints_steps=params.min_eval_frequency)
...
def get_estimator(run_config, params):
	"""Return the model as Tensorflow Estimator or object.
...
def architecture(inputs, is_training, scope='MnistConvNet'):
	...
	Args:
		...
		scope (str): Name of scope of the architecture
	Returns:
	...
	with tf.variable_scope(scope):
		with slim.arg_scope(
				...
				weight_intializer=tf.contrib.layers.xavier_initializer()):
...
# Define data loaders
class IteratorInitializerHook(tf.train.SessionHook):
	"""Hook to initialize data iterator after Session is created. """

	def __init__(self):
		super(IteratorIntializerHook, self).__init__()
...
def get_train_inputs(batch_size, mnist_data):
	...
	def train_inputs():
		...
		with tf.name_scope('Training_data'):
			...
			return next_example, iterator_initializer_hook


def get_test_inputs(batch_size, mnist_data):
	"""Returns the input function to get the test data.
